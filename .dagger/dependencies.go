...
// TicksService returns a ticks worker container as a Dagger service, using the provided Postgres and Temporal services.
func TicksService(
	dag *dagger.Client,
	_ *dagger.Directory,
	db *dagger.Service,
	...
	// Build the ticks container with the official image
	container := dag.Container().From("ghcr.io/cryptellation/ticks")

	// Bind the shared Postgres service to the container
	container = container.WithServiceBinding("postgres", db)
	container = container.WithEnvVariable(
		"SQL_DSN",
		"host=postgres user=cryptellation password=cryptellation dbname=ticks sslmode=disable",
	)
